 ...
 * Gated by usage limits — each enrichment consumes 1 credit
 */
...
export const useEnrichment = () => {
  // Usage limit gating
  const { checkAndExecute, UsageLimitModalComponent, isLimitReached, getRemainingUsage } = useUsageLimit();
  ...
  const enrichByLinkedIn = useCallback(async (input: string): Promise<EnrichedProfileData | null> => {
    // Gate through usage limits before making API call
    ...
    return result;
  }, [resetResult, addToHistory, checkAndExecute]);
