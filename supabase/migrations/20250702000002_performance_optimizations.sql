...
CREATE INDEX IF NOT EXISTS idx_candidates_search_compound 
  ON public.saved_candidates(user_id, status, created_at DESC)
  WHERE status IS NOT NULL;

-- Project-based filtering
CREATE INDEX IF NOT EXISTS idx_project_candidates_performance 
...
-- Agent outputs optimization
CREATE INDEX IF NOT EXISTS idx_agent_outputs_recent 
...
-- Partition search_history by month for better performance
-- Note: This would be implemented in a production environment with more data
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_user_stats_materialized_user_id 
