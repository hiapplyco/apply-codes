-- Phase 2.1: Create normalized entity tables for data quality improvement
...
-- =====================================================
-- Add foreign key references to normalized entities
...
ADD COLUMN IF NOT EXISTS last_enrichment TIMESTAMPTZ;
...
-- Locations indexes  
CREATE INDEX IF NOT EXISTS idx_locations_canonical_name ON public.locations(canonical_name);
...
CREATE INDEX IF NOT EXISTS idx_candidates_company ON public.saved_candidates(company_id);
CREATE INDEX IF NOT EXISTS idx_candidates_location ON public.saved_candidates(location_id);
CREATE INDEX IF NOT EXISTS idx_candidates_experience ON public.saved_candidates(experience_years);
CREATE INDEX IF NOT EXISTS idx_candidates_seniority ON public.saved_candidates(seniority_level);
CREATE INDEX IF NOT EXISTS idx_candidates_enrichment_status ON public.saved_candidates(enrichment_status);
CREATE INDEX IF NOT EXISTS idx_candidates_canonical_linkedin ON public.saved_candidates(canonical_linkedin_url);
CREATE INDEX IF NOT EXISTS idx_candidates_last_enrichment ON public.saved_candidates(last_enrichment);

-- Compound indexes for common query patterns
CREATE INDEX IF NOT EXISTS idx_candidates_compound_location_experience 
  ON public.saved_candidates(location_id, experience_years, seniority_level);
CREATE INDEX IF NOT EXISTS idx_candidates_compound_company_seniority 
  ON public.saved_candidates(company_id, seniority_level);

-- Full-text search indexes for companies and locations
CREATE INDEX IF NOT EXISTS idx_companies_fulltext 
  ON public.companies USING gin(to_tsvector('english', canonical_name || ' ' || COALESCE(array_to_string(aliases, ' '), '')));
CREATE INDEX IF NOT EXISTS idx_locations_fulltext 
  ON public.locations USING gin(to_tsvector('english', canonical_name || ' ' || city || ' ' || COALESCE(state, '') || ' ' || country));

-- Enhanced full-text search for candidates with normalized data
CREATE INDEX IF NOT EXISTS idx_candidates_enhanced_fulltext 
