...
const INPUT_DIR = process.env.INPUT_DIR ||
  path.join(__dirname, '..', 'migration-data', new Date().toISOString().split('T')[0]);
...
async function generateImportSummary(userMapping) {
  ...
  const collections = ['users', 'projects', 'jobs', 'subscriptions', 'billingHistory'];

  for (const collection of collections) {
    try {
      const snapshot = await db.collection(collection).count().get();
      summary.collections[collection] = snapshot.data().count;
    } catch (error) {
...
export { importAuthUsers, importUserProfiles };