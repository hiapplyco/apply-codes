...
CREATE INDEX IF NOT EXISTS idx_user_subscription_details_subscription_id ON user_subscription_details(subscription_id);
CREATE INDEX IF NOT EXISTS idx_user_subscription_details_customer_id ON user_subscription_details(customer_id);
CREATE INDEX IF NOT EXISTS idx_user_subscription_details_status ON user_subscription_details(status);
CREATE INDEX IF NOT EXISTS idx_user_subscription_details_tier ON user_subscription_details(tier);
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        AND tablename = 'user_subscription_details' 
        AND policyname = 'Users can view their own subscription details'
    ) THEN
        CREATE POLICY "Users can view their own subscription details" ON user_subscription_details
            FOR SELECT USING (auth.uid() = user_id);
    END IF;

    -- Users can insert their own subscription details
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'user_subscription_details' 
        AND policyname = 'Users can insert their own subscription details'
    ) THEN
        CREATE POLICY "Users can insert their own subscription details" ON user_subscription_details
            FOR INSERT WITH CHECK (auth.uid() = user_id);
    END IF;

    -- Users can update their own subscription details
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'user_subscription_details' 
        AND policyname = 'Users can update their own subscription details'
    ) THEN
        CREATE POLICY "Users can update their own subscription details" ON user_subscription_details
            FOR UPDATE USING (auth.uid() = user_id);
    END IF;

    -- Service role can manage all subscription details
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' 
        AND tablename = 'user_subscription_details' 
        AND policyname = 'Service role can manage all subscription details'
    ) THEN
...
-- Drop trigger if exists and create new one
DROP TRIGGER IF EXISTS update_user_subscription_details_updated_at ON user_subscription_details;
