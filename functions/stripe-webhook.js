      ...
      switch (event.type) {
        case 'customer.created': {
          ...
          if (customer.metadata?.user_id) {
            ...
            const snapshot = await subscriptionsRef.where('userId', '==', customer.metadata.user_id).get();
        ...
        case 'checkout.session.completed': {
          ...
          if (session.subscription && session.customer && session.metadata?.user_id) {
            ...
            // Update subscription in Firestore
            const subscriptionsRef = db.collection('subscriptions');
            const snapshot = await subscriptionsRef.where('userId', '==', session.metadata.user_id).get();
